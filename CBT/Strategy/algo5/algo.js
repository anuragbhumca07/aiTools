...
// ── Signal — identical to Algo3/Algo4 (EMA Ribbon Swing v2) ──────
...
//
// Checks only:
//   1. Static/trailing SL hit  (position.stopLoss is updated in-place by server.js when trailing)
//   2. $1000 adverse dollar stop
//
// TP is intentionally removed. Trailing SL logic lives in server.js.
// Opposite-signal exit also handled in server.js.
//
...
// ── Trailing SL computation (shared by server.js and backtest) ────
//
// Returns { oldSl, newSl, lockProfit } if SL should advance, null otherwise.
//
// Rules (same for long and short, direction-aware):
//   At unrealPnl >= $300 (band 6): lock in $250 (band 5)
//   At unrealPnl >= $350 (band 7): lock in $300 (band 6)
//   Every additional $50 of profit → SL locks in $50 more
//
function computeTrailUpdate(position, unrealPnl) {
  ...
  if (band < 6) return null; // Not yet at $300 profit threshold

  const lockProfit = (band - 1) * 50; // $250 at band 6, $300 at band 7, ...
...
// Used by the 10-second fast poll so we avoid fetching 250 candles every 10s.
    ...
    const req = https.request(opts, res => {
      ...
      res.on('end', () => {
        try {
          ...
          resolve(parseFloat(json.result[key].c[0])); // c[0] = last trade price
        } catch (e) { reject(e); }
      });
    });
    req.on('error', reject);
