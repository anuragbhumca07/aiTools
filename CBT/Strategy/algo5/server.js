...
const SESSION_SECRET   = process.env.SESSION_SECRET || 'cbt-algo5-dev-secret';
...
function waEntry(side, symbol, timeframe, price, size, sl, tp, riskAmt, balance) {
  ...
  sendWhatsApp(
    `${dir} *[Algo5] ENTRY — ${label} ${sym} ${timeframe}*\n` +
    `Price  : $${f(price)}\n` +
...
function waExit(side, symbol, timeframe, pnl, reason, balance, wins, totalTrades, trailed, lockProfit) {
  ...
  sendWhatsApp(
    `${icon} *[Algo5] EXIT — ${label} ${sym} ${timeframe}*\n` +
    `Reason   : ${reason}\n` +
    `PnL      : *${pnlStr}*\n` +
...
const tickmill = {
  ...
  async init() {
    if (!METAAPI_TOKEN || !METAAPI_ACCOUNT_ID) {
      console.log('[Algo5] MetaApi not configured — running in paper-Kraken mode');
      return;
    }
    try {
      const status = await this._apiGet(`/users/current/accounts/${METAAPI_ACCOUNT_ID}`);
      this.connected = status && status.state === 'deployed';
      console.log(`[Algo5] MetaApi ${this.connected ? '✓ connected' : '✗ not deployed'}`);
    } catch (err) {
      console.error('[Algo5] MetaApi connection error:', err.message);
    }
  },
  ...
  async placeOrder(side, symbol, lots, stopLoss, takeProfit, comment) {
    ...
    try {
      ...
      console.error('[Algo5] placeOrder error:', err.message);
  ...
  async closeOrder(orderId) {
    ...
    try {
      ...
      console.error('[Algo5] closeOrder error:', err.message);
...
function manageFastTicker(sess) {
  ...
  if (needFast && !sess.fastTicker) {
    console.log(`[Algo5] Starting fast 10s poll (unrealPnl=${(pos.unrealizedPnl||0).toFixed(2)})`);
    sess.fastTicker = setInterval(() => fastTick(sess), 10000);
  } else if (!needFast && sess.fastTicker) {
    console.log('[Algo5] Stopping fast poll');
    stopFastTicker(sess);
  }
}
...
async function runFastTick(sess) {
  ...
  try {
    ...
    // Advance trailing SL if profit hit new $50 band
    ...
    if (trailUpdate) {
      ...
      // Regular fast-poll heartbeat while trailing is active
      pushLog(sess, {
    ...
    console.error('[Algo5 fastTick]', err.message);
  }
}
...
async function runTick(sess) {
  ...
  try {
    ...
    if (state.position) {
      ...
      // Start/stop 10s fast ticker based on profit level
      manageFastTicker(sess);
    }
    ...
    if (!state.position && (signal === 'BUY' || signal === 'SELL')) {
      ...
      const orderResult = await tickmill.placeOrder(side, symbol, lots, sl, tp, 'CBT Algo5 Trailing SL');
...
async function runBacktest(symbol, timeframe, months) {
  ...
  for (let i = WINDOW; i < allCandles.length; i++) {
    ...
    if (pos) {
      ...
      // Check SL hit using candle low/high (covers both static and trailing)
      ...
      if (slHit || adverseHit) {
        ...
        // $1000 adverse fires only if it's a worse price than SL
        if (adverseHit && (!slHit ||
...
app.get('/health', (_, res) => res.json({ status: 'ok', strategy: 'swing-v2-trailing' }));
...
app.listen(PORT, () =>
  console.log(`CBT Algo5 (Trailing SL) listening on :${PORT} | MetaApi: ${tickmill.mode}`)
);
