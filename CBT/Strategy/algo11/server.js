...
const SESSION_SECRET   = process.env.SESSION_SECRET || 'cbt-algo11-dev-secret';
...
function waExit(side, symbol, timeframe, pnl, reason, balance, wins, totalTrades, trailed, lockProfit) {
  ...
  sendWhatsApp(
    `${icon} *[Algo11] EXIT — ${label} ${sym} ${timeframe}*\n` +
    `Reason   : ${reason}\n` +
    `PnL      : *${pnlStr}*\n` +
...
const tickmill = {
  ...
  async init() {
    if (!METAAPI_TOKEN || !METAAPI_ACCOUNT_ID) {
      console.log('[Algo11] MetaApi not configured — running in paper-Kraken mode');
      return;
    }
    try {
      const status = await this._apiGet(`/users/current/accounts/${METAAPI_ACCOUNT_ID}`);
      this.connected = status && status.state === 'deployed';
      console.log(`[Algo11] MetaApi ${this.connected ? '✓ connected' : '✗ not deployed'}`);
    } catch (err) {
      console.error('[Algo11] MetaApi connection error:', err.message);
    }
  },
  ...
  async placeOrder(side, symbol, lots, stopLoss, takeProfit, comment) {
    ...
    try {
      ...
      console.error('[Algo11] placeOrder error:', err.message);
  ...
  async closeOrder(orderId) {
    ...
    try {
      ...
      console.error('[Algo11] closeOrder error:', err.message);
...
function manageFastTicker(sess) {
  ...
  if (needFast && !sess.fastTicker) {
    ...
    console.log('[Algo11] Stopping fast poll');
    stopFastTicker(sess);
  }
}
