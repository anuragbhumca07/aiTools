...
const SESSION_SECRET   = process.env.SESSION_SECRET || 'cbt-algo44-dev';
...
function waEntry(side, symbol, price, size, sl, tp, rr, R, regime, balance) {
  ...
  sendWhatsApp(
    `${side==='long'?'🟢':'🔴'} *[Algo44] ENTRY — ${side.toUpperCase()} ${symbol}*\n` +
    `Price  : $${f(price)}\nSize   : ${f(size,4)} lots\n` +
...
function waExit(side, symbol, pnl, reason, trailLockR, balance, wins, total) {
  ...
  sendWhatsApp(
    `${pnl>0?'✅':'❌'} *[Algo44] EXIT — ${side.toUpperCase()} ${symbol}*\n` +
    `Reason  : ${reason}\nPnL     : *${pStr}*\n` +
...
async function get5mCandles(sess) {
  ...
  if (now - sess.cache5m.lastFetch > 4 * 60000) {
    ...
    } catch(e) { console.error('[Algo44] 5m fetch error:', e.message); }
  }
...
async function runBacktest(symbol, months) {
  console.log(`[Algo44] Backtest: ${symbol} ${months}mo 1m+5m`);
...
app.get('/health', (_, res) => res.json({ status:'ok', strategy:'btc-1m-scalp-v1', service:'cbt-algo44' }));
...
app.listen(PORT, () => console.log(`CBT Algo44 (BTC 1m Scalp, algo5-style UI) listening on :${PORT}`));
