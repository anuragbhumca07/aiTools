...
const SESSION_SECRET   = process.env.SESSION_SECRET || 'cbt-algo115-dev-secret';
...
function waEntry(side, symbol, timeframe, price, atr, sl, tp, balance) {
  ...
  sendWhatsApp(
    `${dir} *[Algo115] ENTRY — ${label} ${sym} ${timeframe}*\n` +
    `Price  : $${f(price)}\n` +
...
function waExit(side, symbol, timeframe, pnl, reason, balance, wins, totalTrades, trailed, lockProfit) {
  ...
  sendWhatsApp(
    `${icon} *[Algo115] EXIT — ${label} ${sym} ${timeframe}*\n` +
    `Reason   : ${reason}\n` +
    `PnL      : *${pnlStr}*\n` +
...
const tickmill = {
  ...
  async init() {
    if (!METAAPI_TOKEN || !METAAPI_ACCOUNT_ID) {
      console.log('[Algo115] MetaApi not configured — running in paper-Kraken mode');
      return;
    }
    try {
      const status = await this._apiGet(`/users/current/accounts/${METAAPI_ACCOUNT_ID}`);
      this.connected = status && status.state === 'deployed';
      console.log(`[Algo115] MetaApi ${this.connected ? '✓ connected' : '✗ not deployed'}`);
    } catch (err) {
      console.error('[Algo115] MetaApi connection error:', err.message);
    }
  },
  ...
  async placeOrder(side, symbol, lots, stopLoss, takeProfit, comment) {
    ...
    try {
      ...
      console.error('[Algo115] placeOrder error:', err.message);
  ...
  async closeOrder(orderId) {
    ...
    try {
      ...
      console.error('[Algo115] closeOrder error:', err.message);
...
function manageFastTicker(sess) {
  ...
  if (needFast && !sess.fastTicker) {
    console.log(`[Algo115] Starting fast 10s poll (unrealPnl=${(pos.unrealizedPnl||0).toFixed(2)}, ATR=${(pos.entryAtr||0).toFixed(2)})`);
    sess.fastTicker = setInterval(() => fastTick(sess), 10000);
  } else if (!needFast && sess.fastTicker) {
    console.log('[Algo115] Stopping fast poll');
    stopFastTicker(sess);
  }
}
...
async function runFastTick(sess) {
  ...
  try {
    ...
    if (trailUpdate) {
      ...
      console.log(`[Algo115] TRAIL: ${note}`);
      pushLog(sess, {
    ...
    console.error('[Algo115 fastTick]', err.message);
  }
}
...
async function runTick(sess) {
  ...
  try {
    ...
    if (!state.position && (signal === 'BUY' || signal === 'SELL')) {
      ...
      const orderResult = await tickmill.placeOrder(side, symbol, lots, sl, tp, 'CBT Algo115 Fixed-Qty ATR-Trail');
...
app.listen(PORT, () =>
  console.log(`CBT Algo115 (Fixed Qty=1 · ATR Dynamic SL · 15m) listening on :${PORT} | MetaApi: ${tickmill.mode}`)
);
