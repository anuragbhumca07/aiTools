...
// ── Signal — identical to Algo1 (EMA Ribbon Swing v2) ─────────────
...
// Uses entryAtr stored on position (ATR at the moment of entry).
// qty is fixed at 1, so unrealPnl = price_change in dollars.
//
// Trail milestones (in multiples of entryAtr):
//   unrealPnl >= 1×ATR → SL to break-even (lock $0)
//   unrealPnl >= 2×ATR → lock 1×ATR
//   unrealPnl >= 3×ATR → lock 2×ATR
//   unrealPnl >= N×ATR → lock (N-1)×ATR
//
// SL never retreats. Returns null when no improvement.
function computeTrailUpdate(position, unrealPnl) {
  const { side, entryPrice, stopLoss, entryAtr } = position;
  if (!entryAtr || entryAtr <= 0) return null;
  if (unrealPnl < entryAtr) return null;  // not yet at 1×ATR gain

  const steps = Math.floor(unrealPnl / entryAtr);
  // steps=1→lock $0 (BE), steps=2→lock 1×ATR, steps=3→lock 2×ATR …
  const lockProfit = Math.max(0, (steps - 1) * entryAtr);

  // With fixed size=1, price-lock = lockProfit in dollars
  const newSl = side === 'long'
    ? entryPrice + lockProfit
    : entryPrice - lockProfit;

  const improved = side === 'long' ? newSl > stopLoss : newSl < stopLoss;
  return improved ? { oldSl: stopLoss, newSl, lockProfit, steps } : null;
}
