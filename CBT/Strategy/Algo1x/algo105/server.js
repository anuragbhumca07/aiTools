...
const SESSION_SECRET   = process.env.SESSION_SECRET || 'cbt-algo105-dev-secret';
...
function waEntry(side, symbol, timeframe, price, atr, sl, tp, balance) {
  ...
  sendWhatsApp(
    `${dir} *[Algo105] ENTRY — ${label} ${sym} ${timeframe}*\n` +
    `Price  : $${f(price)}\n` +
...
function waExit(side, symbol, timeframe, pnl, reason, balance, wins, totalTrades, trailed, lockProfit) {
  ...
  sendWhatsApp(
    `${icon} *[Algo105] EXIT — ${label} ${sym} ${timeframe}*\n` +
    `Reason   : ${reason}\n` +
    `PnL      : *${pnlStr}*\n` +
...
const tickmill = {
  ...
  async init() {
    if (!METAAPI_TOKEN || !METAAPI_ACCOUNT_ID) {
      console.log('[Algo105] MetaApi not configured — running in paper-Kraken mode');
      return;
    }
    try {
      const status = await this._apiGet(`/users/current/accounts/${METAAPI_ACCOUNT_ID}`);
      this.connected = status && status.state === 'deployed';
      console.log(`[Algo105] MetaApi ${this.connected ? '✓ connected' : '✗ not deployed'}`);
    } catch (err) {
      console.error('[Algo105] MetaApi connection error:', err.message);
    }
  },
  ...
  async placeOrder(side, symbol, lots, stopLoss, takeProfit, comment) {
    ...
    try {
      ...
      console.error('[Algo105] placeOrder error:', err.message);
  ...
  async closeOrder(orderId) {
    ...
    try {
      ...
      console.error('[Algo105] closeOrder error:', err.message);
...
function defaultState() {
  return {
    running: false, symbol: 'BTCUSDT', timeframe: '5m',
...
function manageFastTicker(sess) {
  ...
  if (needFast && !sess.fastTicker) {
    console.log(`[Algo105] Starting fast 10s poll (unrealPnl=${(pos.unrealizedPnl||0).toFixed(2)}, ATR=${(pos.entryAtr||0).toFixed(2)})`);
    sess.fastTicker = setInterval(() => fastTick(sess), 10000);
  } else if (!needFast && sess.fastTicker) {
    console.log('[Algo105] Stopping fast poll');
    stopFastTicker(sess);
  }
}
...
async function runFastTick(sess) {
  ...
  try {
    ...
    if (trailUpdate) {
      ...
      console.log(`[Algo105] TRAIL: ${note}`);
      pushLog(sess, {
    ...
    console.error('[Algo105 fastTick]', err.message);
  }
}
...
async function runTick(sess) {
  ...
  try {
    ...
    if (!state.position && (signal === 'BUY' || signal === 'SELL')) {
      ...
      const orderResult = await tickmill.placeOrder(side, symbol, lots, sl, tp, 'CBT Algo105 Fixed-Qty ATR-Trail');
...
app.post('/api/backtest', requireAuth, async (req, res) => {
  const { symbol = 'BTCUSDT', timeframe = '5m', months = 3 } = req.body || {};
...
app.listen(PORT, () =>
  console.log(`CBT Algo105 (Fixed Qty=1 · ATR Dynamic SL · 5m) listening on :${PORT} | MetaApi: ${tickmill.mode}`)
);
