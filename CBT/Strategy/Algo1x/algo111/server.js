...
const SESSION_SECRET   = process.env.SESSION_SECRET || 'cbt-algo111-dev-secret';
...
// Fixed position size — always 1 unit of the base asset
const FIXED_SIZE = 1.0;
// Initial SL = 2.0×ATR (wider than algo1's 1.5×ATR to give room with fixed size)
const SL_ATR_MULT = 2.0;
// Initial TP = 4.0×ATR → 2:1 reward-to-risk at entry
...
function waEntry(side, symbol, timeframe, price, atr, sl, tp, balance) {
  const dir   = side === 'long' ? '🟢' : '🔴';
  ...
  sendWhatsApp(
    `${dir} *[Algo111] ENTRY — ${label} ${sym} ${timeframe}*\n` +
    `Price  : $${f(price)}\n` +
    `Size   : ${FIXED_SIZE} ${symbol.replace('USDT', '')} (fixed)\n` +
    `ATR(14): $${f(atr, 2)}\n` +
    `SL     : $${f(sl)}  (${SL_ATR_MULT}×ATR → trail: BE@1×ATR, lock(N-1)×ATR each step)\n` +
    `Init TP: $${f(tp)}  (${TP_ATR_MULT}×ATR)\n` +
    `Balance: $${f(balance)}`
  );
}
...
function waExit(side, symbol, timeframe, pnl, reason, balance, wins, totalTrades, trailed, lockProfit) {
  ...
  sendWhatsApp(
    `${icon} *[Algo111] EXIT — ${label} ${sym} ${timeframe}*\n` +
    `Reason   : ${reason}\n` +
    `PnL      : *${pnlStr}*\n` +
    `Trailing : ${trailed ? `Yes (locked $${parseFloat(lockProfit).toFixed(2)})` : 'No'}\n` +
    `Balance  : $${f(balance)}\n` +
...
`);
['mae REAL DEFAULT 0', 'user_id TEXT DEFAULT "guest"', 'entry_atr REAL DEFAULT 0', 'tickmill_order TEXT'].forEach(col => {
...
const stmtInsert = db.prepare(`
  INSERT INTO trades
    (session_id,user_id,type,side,symbol,timeframe,price,size,pnl,
     stop_loss,take_profit,reason,balance_after,timestamp,mae,entry_atr,tickmill_order)
  VALUES
    (@session_id,@user_id,@type,@side,@symbol,@timeframe,@price,@size,@pnl,
     @stop_loss,@take_profit,@reason,@balance_after,@timestamp,@mae,@entry_atr,@tickmill_order)
`);
...
// ── Strategy registry ──────────────────────────────────────────────
const STRATEGIES = {
  'swing-v3-fixed-qty-atr-trail': {
    name: 'swing-v3-fixed-qty-atr-trail: EMA Ribbon Swing (Fixed Qty=1 · ATR Dynamic SL)',
    description: `EMA21/55/200 + ADX(25) + DI-spread≥15 + 6/7 conditions + Closed Candle Eval + :01s No-Drift Timer + Fixed Qty=1 + ${SL_ATR_MULT}×ATR initial SL + ATR Trail: BE@1×ATR → lock(N-1)×ATR every ATR step (10s fast poll) + Opposite Signal Exit + $1000 Hard Stop`,
  },
};
...
const tickmill = {
  ...
  async init() {
    if (!METAAPI_TOKEN || !METAAPI_ACCOUNT_ID) {
      console.log('[Algo111] MetaApi not configured — running in paper-Kraken mode');
      return;
    }
    try {
      const status = await this._apiGet(`/users/current/accounts/${METAAPI_ACCOUNT_ID}`);
      this.connected = status && status.state === 'deployed';
      console.log(`[Algo111] MetaApi ${this.connected ? '✓ connected' : '✗ not deployed'}`);
    } catch (err) {
      console.error('[Algo111] MetaApi connection error:', err.message);
    }
  },
  ...
  async placeOrder(side, symbol, lots, stopLoss, takeProfit, comment) {
    ...
    try {
      ...
      console.error('[Algo111] placeOrder error:', err.message);
  ...
  async closeOrder(orderId) {
    ...
    try {
      ...
      console.error('[Algo111] closeOrder error:', err.message);
...
function defaultState() {
  return {
    ...
    strategyId: 'swing-v3-fixed-qty-atr-trail', mode: 'paper',
...
async function handleExit(sess, position, exitPrice, exitReasonStr, indicators, ts) {
  ...
  const { side, entryPrice, size, stopLoss, takeProfit, mae, entryAtr, tickmillOrderId, trailing, trailLockProfit } = position;
  ...
  const trade = {
    ...
    entry_atr: parseFloat((entryAtr || 0).toFixed(4)),
...
function manageFastTicker(sess) {
  ...
  // Start fast poll once unrealPnl reaches 0.5×entryAtr (approaching first trail step)
  const halfAtr = pos && pos.entryAtr ? 0.5 * pos.entryAtr : 30;
  const needFast = state.running && pos &&
    ((pos.unrealizedPnl || 0) > halfAtr || pos.trailing);

  if (needFast && !sess.fastTicker) {
    console.log(`[Algo111] Starting fast 10s poll (unrealPnl=${(pos.unrealizedPnl||0).toFixed(2)}, ATR=${(pos.entryAtr||0).toFixed(2)})`);
    sess.fastTicker = setInterval(() => fastTick(sess), 10000);
  } else if (!needFast && sess.fastTicker) {
    console.log('[Algo111] Stopping fast poll');
    stopFastTicker(sess);
  }
}
...
async function runFastTick(sess) {
  ...
  try {
    ...
    const pos   = state.position;
    const { side, entryPrice, size, stopLoss, entryAtr } = pos;
    ...
    if (slHit) {
      const lockStr = pos.trailing ? ` (locked $${parseFloat(pos.trailLockProfit||0).toFixed(2)})` : '';
      const reason = pos.trailing
        ? `Trailing SL hit: $${pos.stopLoss.toFixed(2)}${lockStr}`
    ...
    if (trailUpdate) {
      const { oldSl, newSl, lockProfit, steps } = trailUpdate;
      pos.stopLoss        = newSl;
      pos.trailing        = true;
      pos.trailLockProfit = lockProfit;

      const atrStr = entryAtr ? ` (${steps}×ATR, entry ATR=$${entryAtr.toFixed(2)})` : '';
      const note = `SL: $${oldSl.toFixed(2)} → $${newSl.toFixed(2)} | locked $${lockProfit.toFixed(2)}${atrStr}`;
      console.log(`[Algo111] TRAIL: ${note}`);
      pushLog(sess, {
      ...
      pushLog(sess, {
        ...
        indicators: { ...(state.lastIndicators || {}), price },
        reason: [`Trail active — locked $${parseFloat(pos.trailLockProfit||0).toFixed(2)}, SL: $${pos.stopLoss.toFixed(2)}, PnL: $${unrealPnl.toFixed(2)}, ATR: $${(entryAtr||0).toFixed(2)}`],
      });
    }

    broadcast(sess, { type: 'tick', state: publicState(state) });
  } catch (err) {
    console.error('[Algo111 fastTick]', err.message);
  }
}
...
async function runTick(sess) {
  ...
  try {
    ...
    // ── SL / $1000 adverse exit — uses live price ─────────────────
    if (state.position) {
      ...
      if (exitResult.exit) {
        ...
        const trade = {
          ...
          entry_atr: parseFloat((pos.entryAtr || 0).toFixed(4)),
        ...
        pushLog(sess, { ts, type: 'EXIT', side: pos.side, price: exitPrice, pnl,
                        ...
                        entryAtr: (pos.entryAtr || 0).toFixed(2),
      ...
      // Advance trailing SL on candle close (belt-and-suspenders)
      ...
      if (trailUpdate) {
        ...
        const atrStr = state.position.entryAtr ? ` (${steps}×ATR, ATR=$${state.position.entryAtr.toFixed(2)})` : '';
        pushLog(sess, {
          ts, type: 'TICK', signal: 'TRAIL-UPDATE', price: livePrice,
          indicators: exitResult ? exitResult.indicators : state.lastIndicators,
          reason: [`SL: $${oldSl.toFixed(2)} → $${newSl.toFixed(2)} | locked $${lockProfit.toFixed(2)}${atrStr}`],
        });
      }
    ...
    // ── Entry logic — fixed size=1, ATR-based SL ──────────────────
    if (!state.position && (signal === 'BUY' || signal === 'SELL')) {
      const { atr } = indicators;
      const side     = signal === 'BUY' ? 'long' : 'short';
      const stopDist = SL_ATR_MULT * atr;
      const sl       = side === 'long' ? price - stopDist : price + stopDist;
      const tp       = side === 'long' ? price + TP_ATR_MULT * atr : price - TP_ATR_MULT * atr;
      // lots for MetaApi (0.01 = 1 BTC on most MT5 brokers)
      const lots     = parseFloat((FIXED_SIZE * 0.01).toFixed(2));

      const orderResult = await tickmill.placeOrder(side, symbol, lots, sl, tp, 'CBT Algo111 Fixed-Qty ATR-Trail');

      state.position = {
        side, entryPrice: price, size: FIXED_SIZE, stopLoss: sl, takeProfit: tp,
        entryTime: ts, unrealizedPnl: 0, mae: 0,
        trailing: false, trailLockProfit: 0,
        entryAtr: atr,          // ATR at entry — used for all trailing calculations
      ...
      const trade = {
        ...
        balance_after: parseFloat(state.balance.toFixed(4)),
        timestamp: ts, mae: 0,
        entry_atr: parseFloat(atr.toFixed(4)),
        tickmill_order: orderResult.orderId,
      };
      stmtInsert.run(trade);
      waEntry(side, symbol, timeframe, price, atr, sl, tp, state.balance);
      pushLog(sess, { ts, type: 'ENTRY', side, signal, price, size: FIXED_SIZE,
                      stopLoss: sl, takeProfit: tp, entryAtr: atr,
...
async function runBacktest(symbol, timeframe, months) {
  ...
  for (let i = WINDOW; i < allCandles.length; i++) {
    ...
    if (pos) {
      // Use candle high/low to detect if SL or trailing SL was touched intra-bar
      ...
      // Advance trailing SL based on best price seen this bar
      ...
      if (slHit || adverseHit) {
        ...
        if (adverseHit && (!slHit ||
          ...
          const lockStr = pos.trailing ? ` (locked $${parseFloat(pos.trailLockProfit||0).toFixed(2)})` : '';
          reason    = pos.trailing
            ? `Trailing SL hit: $${exitPrice.toFixed(2)}${lockStr}`
    ...
    if (!pos && (sig.signal === 'BUY' || sig.signal === 'SELL')) {
      ...
      pos = {
        ...
        takeProfit:     newSide === 'long' ? price + TP_ATR_MULT * atr : price - TP_ATR_MULT * atr,
  ...
  const avgAtr = trades.length
    ? trades.reduce((s, t) => s + (t.entryAtr || 0), 0) / trades.length
    : 0;
...
app.get('/health', (_, res) => res.json({ status: 'ok', strategy: 'swing-v3-fixed-qty-atr-trail' }));
...
app.listen(PORT, () =>
  console.log(`CBT Algo111 (Fixed Qty=1 · ATR Dynamic SL) listening on :${PORT} | MetaApi: ${tickmill.mode}`)
);
