...
// ── Exit check ────────────────────────────────────────────────────
// Uses position.adverseStop (price-distance hard stop, defaults 1000).
// Scaled at entry to 5×ATR so it fits any account size proportionally.
function checkExit(position, candles) {
  const ind = computeIndicators(candles);
  const { price } = ind;
  const { side, entryPrice, stopLoss } = position;
  const adverseStop = position.adverseStop || 1000;

  const reasons = [];

  if (side === 'long') {
    if (price <= stopLoss)                 reasons.push(`SL hit: ${price.toFixed(2)} ≤ ${stopLoss.toFixed(2)}`);
    if (price <= entryPrice - adverseStop) reasons.push(`Adverse stop: ${price.toFixed(2)} ≤ ${(entryPrice - adverseStop).toFixed(2)}`);
  } else {
    if (price >= stopLoss)                 reasons.push(`SL hit: ${price.toFixed(2)} ≥ ${stopLoss.toFixed(2)}`);
    if (price >= entryPrice + adverseStop) reasons.push(`Adverse stop: ${price.toFixed(2)} ≥ ${(entryPrice + adverseStop).toFixed(2)}`);
  }
...
// position.trailTrigger: dollar profit threshold to start trailing (default 300)
// position.trailStep:    dollar step to advance SL each band (default 50)
//
// Scaled at entry: trailTrigger = 2 × initialRisk, trailStep = 0.3 × initialRisk
//
function computeTrailUpdate(position, unrealPnl) {
  const { side, entryPrice, size, stopLoss } = position;
  const trigger = position.trailTrigger || 300;
  const step    = position.trailStep    || 50;

  if (unrealPnl < trigger) return null;

  const stepsAbove = Math.floor((unrealPnl - trigger) / step);
  const lockProfit = Math.max(0, (trigger - step) + stepsAbove * step);
...
// ── MetaAPI / Tickmill data fetchers (primary) ────────────────────

const MA_TOKEN   = process.env.METAAPI_TOKEN      || '';
const MA_ACCOUNT = process.env.METAAPI_ACCOUNT_ID || '';
const MA_REGION  = process.env.METAAPI_REGION     || 'new-york';
...
function toTickmillSymbol(sym) {
  return sym.replace('USDT', 'USD').replace('BUSD', 'USD');
}

function metaapiGet(path) {
  return new Promise((resolve, reject) => {
    const opts = {
      hostname: `mt-client-api-v1.${MA_REGION}.agiliumtrade.ai`,
      path,
      method: 'GET',
...
async function fetchCandlesMetaAPI(symbol, interval, limit) {
  const tkSym = toTickmillSymbol(symbol);
  const tf    = MA_TF_MAP[interval] || '4H';
  const data  = await metaapiGet(
    `/users/current/accounts/${MA_ACCOUNT}/historical-market-data/symbols/${encodeURIComponent(tkSym)}/timeframes/${tf}/candles?limit=${limit}`
  );
  if (!Array.isArray(data)) throw new Error(`MetaAPI candles error: ${JSON.stringify(data).slice(0, 200)}`);
  return [...data].sort((a, b) => new Date(a.time) - new Date(b.time)).map(mapMetaCandle);
}

async function fetchCandlesHistoricalMetaAPI(symbol, interval, months) {
  const tkSym = toTickmillSymbol(symbol);
  const tf    = MA_TF_MAP[interval] || '4H';
  const ivMs  = CANDLE_MS_MAP[interval] || 14400000;
  const now   = Date.now();
  const from  = now - Math.ceil(months * 30.44 * 24 * 3600000) - 100 * ivMs;

  const allCandles = [];
  let startTime    = new Date(from).toISOString();

  for (let i = 0; i < 20; i++) {
    const data = await metaapiGet(
      `/users/current/accounts/${MA_ACCOUNT}/historical-market-data/symbols/${encodeURIComponent(tkSym)}/timeframes/${tf}/candles?startTime=${encodeURIComponent(startTime)}&limit=1000`
    );
    if (!Array.isArray(data) || !data.length) break;
    const sorted = [...data].sort((a, b) => new Date(a.time) - new Date(b.time));
    const seen   = new Set(allCandles.map(c => c.time));
    const mapped = sorted.map(mapMetaCandle).filter(c => !seen.has(c.time));
    if (!mapped.length) break;
    allCandles.push(...mapped);
    const lastMs = allCandles[allCandles.length - 1].time;
    if (lastMs >= now - ivMs * 3) break;
    startTime = new Date(lastMs + ivMs).toISOString();
    if (i < 19) await new Promise(r => setTimeout(r, 350));
  }
  return allCandles;
}

async function fetchCurrentPriceMetaAPI(symbol) {
  const tkSym = toTickmillSymbol(symbol);
  const data  = await metaapiGet(
    `/users/current/accounts/${MA_ACCOUNT}/historical-market-data/symbols/${encodeURIComponent(tkSym)}/timeframes/1M/candles?limit=2`
  );
  if (!Array.isArray(data) || !data.length) throw new Error('No price data from MetaAPI');
  const sorted = [...data].sort((a, b) => new Date(a.time) - new Date(b.time));
  return sorted[sorted.length - 1].close;
}

// ── Kraken fallback (paper mode, no MetaAPI credentials) ──────────
...
// ── Public data API (MetaAPI primary, Kraken fallback) ────────────

async function fetchCandles(symbol, interval, limit = 200) {
  if (MA_TOKEN && MA_ACCOUNT) return fetchCandlesMetaAPI(symbol, interval, limit);
...
async function fetchCandlesHistorical(symbol, interval, months) {
  if (MA_TOKEN && MA_ACCOUNT) return fetchCandlesHistoricalMetaAPI(symbol, interval, months);
  const pair  = KRAKEN_PAIR[symbol] || symbol;
  const ivMin = KRAKEN_INTERVAL[interval] || 60;
...
async function fetchCurrentPrice(symbol) {
  if (MA_TOKEN && MA_ACCOUNT) return fetchCurrentPriceMetaAPI(symbol);
  const pair = KRAKEN_PAIR[symbol] || symbol;
  return new Promise((resolve, reject) => {
    const opts = {
