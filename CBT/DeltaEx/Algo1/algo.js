...
// ── EMA Ribbon Swing v3 — pure strategy logic (no data fetching) ───
// Identical signal logic to CBT/Strategy/algo1 but data-source agnostic.
...
// ── Signal — EMA Ribbon Swing v3 (Closed Candle) ─────────────────
function generateSignal(candles) {
  ...
  if (atrMA20 !== null && atr > atrMA20 * 1.3)
    return { signal: 'HOLD', reason: [`ATR spike: ATR(${fmt(atr, 2)}) > 1.3×ATR_MA(${fmt(atrMA20, 2)})`], indicators: ind, buyScore, sellScore };
  const ema21Dist = Math.abs(price - ema21);
  if (ema21Dist > atr * 1.5)
    return { signal: 'HOLD', reason: [`Price overextended: |price−EMA21|(${fmt(ema21Dist, 2)}) > 1.5×ATR(${fmt(atr * 1.5, 2)})`], indicators: ind, buyScore, sellScore };
...
// ── Exit check — SL hit or $1000 adverse ─────────────────────────
function checkExit(position, candles) {
  ...
  const reasons = [];
  if (side === 'long') {
    if (price <= stopLoss)          reasons.push(`SL hit: ${price.toFixed(4)} ≤ ${stopLoss.toFixed(4)}`);
    if (price <= entryPrice - 1000) reasons.push(`$1000 adverse: ${price.toFixed(4)} ≤ ${(entryPrice - 1000).toFixed(4)}`);
  } else {
    if (price >= stopLoss)          reasons.push(`SL hit: ${price.toFixed(4)} ≥ ${stopLoss.toFixed(4)}`);
    if (price >= entryPrice + 1000) reasons.push(`$1000 adverse: ${price.toFixed(4)} ≥ ${(entryPrice + 1000).toFixed(4)}`);
  }
...
// ── Trailing SL (PnL-based milestones) ───────────────────────────
// profit ≥ $200 → SL to break-even (lock $0)
// profit ≥ $250 → lock $100
// profit ≥ $300 → lock $200
// profit ≥ $400 → lock $300  (every +$100 above $300: lockProfit = profit − $100)
function computeTrailUpdate(position, unrealPnl) {
  const { side, entryPrice, size, stopLoss } = position;
  let lockProfit;
  if (unrealPnl >= 300) {
    lockProfit = Math.floor((unrealPnl - 100) / 100) * 100;
